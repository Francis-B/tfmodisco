...
def _laplacian_null(track, window_size, num_to_samp, random_seed=1234):
    ...
    neg_laplace_lambda = np.max(
        -np.log(1 - (percentiles_to_use / 100.0))
        / (np.abs(np.percentile(a=neg_values, q=100 - percentiles_to_use) - mu))
    )
...
def _iterative_extract_seqlets(score_track, window, flank, center=True):
    ...
    for example_idx, single_score_track in enumerate(score_track):
        ...
        while True:
            ...
            r_idx = int(min(np.ceil(argmax + 0.5 + suppress), length))
...
def _isotonic_thresholds(
    ...
    precisions = np.minimum(
        np.maximum(
            1
            + implied_frac_neg * (1 - (1 / np.maximum(model.transform(values), 1e-7))),
            0.0,
        ),
        1.0,
    )
...
def extract_seqlets(
    ...
    neg_threshold = _isotonic_thresholds(
        neg_values, neg_null_values, increasing=False, target_fdr=target_fdr
    )
    ...
    distribution = np.array(sorted(np.abs(np.concatenate(smoothed_tracks, axis=0))))

    transformed_pos_threshold = (
        np.sign(pos_threshold)
        * np.searchsorted(a=distribution, v=abs(pos_threshold))
        / len(distribution)
    )

    transformed_neg_threshold = (
        np.sign(neg_threshold)
        * np.searchsorted(a=distribution, v=abs(neg_threshold))
        / len(distribution)
    )
    ...
    if isinstance(smoothed_tracks, list):
        for smoothed_track in smoothed_tracks:
            ...
            smoothed_track[idxs] = np.abs(smoothed_track[idxs])
            smoothed_track[~idxs] = -np.inf

            # Filter out the flanks
            smoothed_track[:flank] = -np.inf
            smoothed_track[-flank:] = -np.inf
    ...
    weak_thresh = min(
        min(transformed_pos_threshold, abs(transformed_neg_threshold)) - 0.0001,
        weak_threshold_for_counting_sign,
    )
