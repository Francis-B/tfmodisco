...
# Temporary fix to silent pyright that is having a hard time with h5py
    ...
    if "metacluster_0" in old_grp.keys():
        ...
        if "patterns" in old_patterns_grp:
            ...
            for pattern in old_patterns_grp["all_pattern_names"][:]:
                ...
                hypothetical_contribs = old_pattern["task0_hypothetical_contribs"][
                    "fwd"
                ][:]
                ...
                pattern_grp.create_dataset(
                    "hypothetical_contribs", data=hypothetical_contribs
                )
                ...
                if "subcluster_to_subpattern" in old_pattern.keys():
                    ...
                    for subpattern in old_subpatterns_grp["subcluster_names"][:]:
                        ...
                        hypothetical_contribs = old_subpattern[
                        ...
                        subpattern_grp.create_dataset(
                            "contrib_scores", data=contrib_scores
                        )
                        subpattern_grp.create_dataset(
                            "hypothetical_contribs", data=hypothetical_contribs
                        )
                        ...
                        seqlet_grp.create_dataset(
                            "n_seqlets",
                            data=np.array(
                                [len(old_subpattern["seqlets_and_alnmts"]["seqlets"])]
                            ),
                        )
...
def save_pattern(pattern, grp):
    ...
    seqlet_grp.create_dataset(
        "start", data=np.array([seqlet.start for seqlet in pattern.seqlets])
    )
    seqlet_grp.create_dataset(
    ...
    seqlet_grp.create_dataset(
        "is_revcomp", data=np.array([seqlet.is_revcomp for seqlet in pattern.seqlets])
    )

    seqlet_grp.create_dataset(
        "sequence", data=np.array([seqlet.sequence for seqlet in pattern.seqlets])
    )
    seqlet_grp.create_dataset(
        "contrib_scores",
...
def write_meme_from_h5(
    filename: os.PathLike,
    datatype: util.MemeDataType,
    output_filename: Union[os.PathLike, None],
    is_quiet: bool,
) -> None:
        ...
        for pattern_group in ["pos_patterns", "neg_patterns"]:
            ...
            for name, datasets in grp[pattern_group].items():
                ...
                if datatype == util.MemeDataType.PFM:
                    probability_matrix = datasets["sequence"][:] / np.sum(  # type: ignore
                    ...
                    probability_matrix = scipy.special.softmax(
                        datasets["contrib_scores"][:], axis=1
                    )
                elif datatype == util.MemeDataType.hCWM_PFM:
                    # Softmax version of hCWM.
                    probability_matrix = scipy.special.softmax(
                        datasets["hypothetical_contribs"][:], axis=1
                    )
                else:
...
def write_bed_from_h5(
    modisco_results_filepath: os.PathLike,
    peaks_filepath: os.PathLike,
    output_filepath: os.PathLike,
    valid_chroms: Union[List[str], Literal["*"]],
    ...
    with open(peaks_filepath, "r") as peaks_file:
        ...
        peak_rows_filtered = (
            util.filter_bed_rows_by_chrom(peak_rows, valid_chroms)
            if valid_chroms != "*"
            else peak_rows
        )
    ...
    with h5py.File(modisco_results_filepath, "r") as grp:
        ...
        for contribution_dir in ["pos", "neg"]:
            ...
            for pattern_name, datasets in grp[patterns_category].items():
                    ...
                    track_line=bed_writer.BEDTrackLine(
                        arguments=OrderedDict(
                            [
                                ...
                                (
                                    "description",
                                    f"TF-MoDISco pattern '{pattern_name}' on the positive strand.",
                                ),
                            ]
                        )
                    )
                ...
                assert (
                    datasets["seqlets"]["start"].shape[0]
                    == datasets["seqlets"]["end"].shape[0]
                )
                ...
                for idx in range(datasets["seqlets"]["start"].shape[0]):
                    ...
                    absolute_seqlet_start = (
                        absolute_peak_center
                        - window_center_offset
                        + seqlet_start_offset
                    )
                    absolute_seqlet_end = (
                        absolute_peak_center - window_center_offset + seqlet_end_offset
                    )

                    strand_char = (
                        "-"
                        if bool(datasets["seqlets"]["is_revcomp"][idx]) is True
                        else "+"
                    )
...
def write_fasta_from_h5(
    modisco_results_filepath: os.PathLike,
    peaks_filepath: os.PathLike,
    sequences_file: os.PathLike,
    output_filepath: Union[os.PathLike, None],
    valid_chroms: Union[List[str], Literal["*"]],
    window_size: Union[None, int],
    is_quiet: bool,
) -> None:
    ...
    with h5py.File(modisco_results_filepath, "r") as grp:
        ...
        if window_size is None:
            if "window_size" not in grp.attrs:
                raise ValueError(
                    "window_size must be specified either in the h5 file or as an argument. Older versions of modisco does not store `window_size` in the h5 file."
                )
            window_size = int(grp.attrs["window_size"])
...
def convert_new_to_old(new_format_filename, old_format_filename):
    ...
    patterns_group_name_to_metacluster_name = {
        "pos_patterns": "metacluster_0",
        "neg_patterns": "metacluster_1",
    }

    for patterns_group_name in ["pos_patterns", "neg_patterns"]:
        if patterns_group_name in new_f.keys():
            metacluster_name = patterns_group_name_to_metacluster_name[
                patterns_group_name
            ]
            ...
            old_patterns_grp = old_metacluster_grp.create_group(
            ...
            if len(new_patterns_grp.keys()) > 0:
                ...
                for pattern in pattern_names:
                    ...
                    old_pattern_grp.create_dataset(
                        "task0_contrib_scores/fwd", data=contrib_scores
                    )
                    old_pattern_grp.create_dataset(
                        "task0_hypothetical_contribs/fwd", data=hypothetical_contribs
                    )
                    ...
                    # old format
                    old_seq_align_grp = old_pattern_grp.create_group(
                        "seqlets_and_alnmts"
                    )
                    ...
                    old_seq_align_grp.create_dataset(
                        "alnmts", data=np.zeros((len(seqlet_strings),)), dtype="i"
                    )

                    # repeat the process for each pattern/cluster (above) for each subpattern/subcluster (below)

                    subcluster_names = [
                        k for k in pattern_grp.keys() if k.startswith("subcluster_")
                    ]
                    if len(subcluster_names) > 0:
                        old_subpatterns_grp = old_pattern_grp.create_group(
                            "subcluster_to_subpattern"
                        )
                        ...
                        for subpattern in subcluster_names:
                            ...
                            hypothetical_contribs = subpattern_grp[
                                "hypothetical_contribs"
                            ]

                            old_subpattern_grp = old_subpatterns_grp.create_group(
                                subpattern
                            )
                            old_subpattern_grp.create_dataset(
                                "sequence/fwd", data=sequence
                            )
                            old_subpattern_grp.create_dataset(
                                "task0_contrib_scores/fwd", data=contrib_scores
                            )
                            old_subpattern_grp.create_dataset(
                                "task0_hypothetical_contribs/fwd",
                                data=hypothetical_contribs,
                            )
                            ...
                            old_seq_align_grp = old_subpattern_grp.create_group(
                                "seqlets_and_alnmts"
                            )
                            old_seq_align_grp.create_dataset(
                                "seqlets", data=seqlet_strings
                            )
                            old_seq_align_grp.create_dataset(
                                "alnmts",
                                data=np.zeros((len(seqlet_strings),)),
                                dtype="i",
                            )

                        old_subpatterns_grp.create_dataset(
                            "subcluster_names", data=subcluster_names
                        )

                old_patterns_subgrp.create_dataset(
                    "all_pattern_names", data=pattern_names
                )

            # required to avoid error: a collection of seqlets for the entire metacluster
            old_metacluster_grp.create_dataset(
                "seqlets", data=metacluster_seqlet_strings
            )
