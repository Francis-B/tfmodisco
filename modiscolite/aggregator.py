...
def polish_pattern(pattern, min_frac, min_num, track_set, flank, window_size, bg_freq):
    ...
    start_idx = np.argmax(
        util.cpu_sliding_window_sum(arr=per_pos_ic, window_size=window_size)
    )
...
def _align_patterns(
    ...
    best_crossmetric, best_crossmetric_argmax = metric(
...
def merge_in_seqlets_filledges(
    ...
    for seqlet in seqlets_to_merge:
        ...
        parent_right_expansion = max(
            0, (alnmt + preexpansion_seqletlen) - len(parent_pattern)
        )
...
def _detect_spurious_merging(
    ...
    for i, pattern in enumerate(
        ...
        if len(pattern.seqlets) > min_in_subcluster:
            ...
            refined_subpatterns = SimilarPatternsCollapser(
    ...
    return SimilarPatternsCollapser(
        patterns=to_return,
        track_set=track_set,
        min_overlap=min_overlap,
        prob_and_pertrack_sim_merge_thresholds=prob_and_pertrack_sim_merge_thresholds,
...
def SimilarPatternsCollapser(
    ...
    current_level_nodes = [PatternMergeHierarchyNode(pattern=x) for x in patterns]
    ...
    while merge_occurred_last_iteration:
        ...
        for i in range(n):
            for j in range(n):
                ...
                alnmt, rc, aligner_sim = _align_patterns(
                    parent_pattern=patterns[i],
                ...
                pattern1_fwdseqdata, _ = util.get_2d_data_from_patterns(
                    subsample_patterns[i].seqlets
                )

                pattern2_fwdseqdata, _ = util.get_2d_data_from_patterns(
                    pattern2_shifted_seqlets
                )
        ...
        indices_to_merge = []
        merge_partners_so_far = dict([(i, set([i])) for i in range(len(patterns))])

        # merge patterns with highest similarity first
        sorted_pairs = sorted(
            [
                (i, j, patterns_to_patterns_aligner_sim[i, j])
        ...
        for i, j, aligner_sim in sorted_pairs:
            ...
            cross_contam = min(cross_contamination[i, j], cross_contamination[j, i])

            collapse = any(
                [
                    (cross_contam >= x[0] and aligner_sim >= x[1])
            ...
            if collapse:
                ...
                merge_under_consideration = set(
                    list(merge_partners_so_far[i]) + list(merge_partners_so_far[j])
                )

                for m1 in merge_under_consideration:
                    for m2 in merge_under_consideration:
                        if m1 < m2:
                            cross_contam_here = 0.5 * (
                                cross_contamination[m1, m2]
                                + cross_contamination[m2, m1]
                            )
                            aligner_sim_here = patterns_to_patterns_aligner_sim[m1, m2]

                            dealbreaker = any(
                                [
                                    (
                                        cross_contam_here <= x[0]
                                        and aligner_sim_here <= x[1]
                                    )
                ...
                if collapse_passed:
                    ...
                    for an_idx in merge_under_consideration:
                        merge_partners_so_far[an_idx] = merge_under_consideration
            ...
            if pattern1 != pattern2:  # if not the same object
                ...
                if new_pattern is not None:
                    for k in range(len(patterns)):
                        ...
                        if (
                            patterns[k] == parent_pattern
                            or patterns[k] == child_pattern
                        ):
        ...
        if merge_occurred_last_iteration:
            ...
            for old_pattern_idx, (old_pattern_node, corresp_new_pattern) in enumerate(
                zip(current_level_nodes, old_to_new_pattern_mapping)
            ):
            ...
            for old_idx_i, new_idx_i in old_to_new_index_mappings.items():
                for old_idx_j, new_idx_j in old_to_new_index_mappings.items():
                    new_pairwise_aurocs[new_idx_i, new_idx_j] = pairwise_aurocs[
                        old_idx_i, old_idx_j
                    ]
                    new_pairwise_sims[new_idx_i, new_idx_j] = pairwise_sims[
                        old_idx_i, old_idx_j
                    ]
            ...
            for next_level_node in next_level_nodes:
                ...
                for old_pattern_idx, (
                    old_pattern_node,
                    corresp_new_pattern,
                ) in enumerate(zip(current_level_nodes, old_to_new_pattern_mapping)):
                    ...
                    if old_pattern_node.pattern != corresp_new_pattern:
                        if next_level_node.pattern == corresp_new_pattern:
                            ...
                            # indices_merged_with
                            indices_merged = tuple(
                                sorted(merge_partners_so_far[old_pattern_idx])
                            )
                            # get the relevant slice
                            submat_crosscontam = cross_contamination[indices_merged, :][
                                :, indices_merged
                            ]
                            submat_alignersim = patterns_to_patterns_aligner_sim[
                                indices_merged, :
                            ][:, indices_merged]

                            if next_level_node.indices_merged is not None:
                                assert (
                                    next_level_node.indices_merged == indices_merged
                                ), (next_level_node.indices_merged, indices_merged)
                            else:
                                next_level_node.indices_merged = indices_merged
                                next_level_node.submat_crosscontam = submat_crosscontam
                                next_level_node.submat_alignersim = submat_alignersim

                            next_level_node.child_nodes.append(old_pattern_node)
