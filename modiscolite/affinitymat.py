...
@njit(
    "float64(float64[:], int64[:], int64[:], float64[:], int64[:], int64[:], int64, int64)"
)
...
def _sparse_mm_dot(
    ...
    for i in prange(n_rows):
        ...
        for j in range(n_rows):
            xdot = _sparse_vv_dot(
                X_data, X_indices, X_indptr, X_data, X_indices, X_indptr, i, j
            )
            if stranded:
                dot[j] = xdot
            else:
                ydot = _sparse_vv_dot(
                    X_data, X_indices, X_indptr, Y_data, Y_indices, Y_indptr, i, j
                )
                dot[j] = max(xdot, ydot)
...
def cosine_similarity_from_seqlets(
    seqlets,
    n_neighbors,
    sign,
    topn=20,
    ...
    X_fwd = gapped_kmer._seqlet_to_gkmers(
        seqlets, topn, min_k, max_k, max_gap, max_len, max_entries, True, sign
    )

    X_bwd = gapped_kmer._seqlet_to_gkmers(
        seqlets, topn, min_k, max_k, max_gap, max_len, max_entries, False, sign
    )
...
def jaccard_from_seqlets(
    ...
    if filter_seqlets is None:
        ...
        filters_all_fwd_data, filters_all_rev_data = util.get_2d_data_from_patterns(
            filter_seqlets
        )

    if seqlet_neighbors is None:
        seqlet_neighbors = [list(range(len(filter_seqlets))) for x in seqlets]
...
def jaccard(
    ...
    scores = np.zeros(
        (Y.shape[0], seqlet_neighbors.shape[1], len_output), dtype="float32"
    )
    total = Y.shape[0]
...
def pearson_correlation(X, Y, min_overlap=None, func=np.ceil):
    ...
    for idx in range(len_output):
        ...
        scores_ = np.dot(
            (X / np.linalg.norm(X)).ravel(), (Y_ / np.linalg.norm(Y_)).ravel()
        )
...
class NNTsneConditionalProbs:
    ...
    def __call__(self, affinity_mat, nearest_neighbors):
        ...
        P = self.tsne_probs_calc(
            distances_nn=distmat_nn[:, 1 : (k + 1)],
            neighbors_nn=[row[1 : (k + 1)] for row in nearest_neighbors],
        )
        return P
    ...
    def tsne_probs_calc(self, distances_nn, neighbors_nn):
        ...
        P = coo_matrix((data, (rows, cols)), shape=(len(neighbors), len(neighbors)))
        return P
